...
export function useBotEngine() {
    ...
    const parseNaturalLanguageOrder = (text, lang) => {
        ...
        // Pattern 1: Quantity + item/category
        const intentRegex = new RegExp(`(${numberPattern})\\s+(?:of\\s+|for\\s+|add\\s+|give\\s+me\\s+)?([a-zA-Z\\s\\u0600-\\u06FF]+?)(?=\\s*(?:\\d|and|&|,|\\+|$|\\?))`, 'gi');
        
        // Pattern 2: Item/category without quantity (e.g., "and wraps", "and burger")
        ...
        while ((match = intentRegex.exec(cleanText)) !== null) {
            ...
            if (item) {
                ...
                if (nameEn === search || nameEn.includes(search) || search.includes(nameEn)) {
                    detectedIntents.push({ type: 'ITEM', data: item, qty });
                    continue;
                }
                
                // If fuzzy match distance is very small (≤2), likely an item
                const dist = levenshtein(search, nameEn);
                if (dist <= 2) {
                    detectedIntents.push({ type: 'ITEM', data: item, qty });
                    continue;
                }
        ...
        // Second pass: Catch items without quantities (e.g., "and wrapsssss", "and burger")
        // Reset regex

...
export function useBotEngine() {
    ...
    const processInput = (text) => {
        ...
        // Process NLP intents FIRST (before state-based handlers)
        // This ensures category/item requests work regardless of current state
        const nlpIntents = parseAdvancedNLP(text, 'en');
        if (nlpIntents.length > 0) {
            const firstIntent = nlpIntents[0];

            // Handle CATEGORY intent (e.g., "burger", "menu burger", "show me burger")
            if (firstIntent.type === 'CATEGORY') {
                const categoryId = firstIntent.data.id;
                let allItems = [];

                // Get items from this category
                const items = menu.items[categoryId] || [];
                items.forEach(item => {
                    allItems.push({ ...item, catId: categoryId });
                });
                ...
                addMessage(`Here are our ${firstIntent.data.title.en}:`, 'bot');
            ...
            // Handle ITEM intent with smart flow
            if (firstIntent.type === 'ITEM') {
                const item = firstIntent.data;
                const qty = firstIntent.qty;
                const preference = firstIntent.preference;
                const needsSpicy = item.catId === 'burgers' || item.catId === 'wraps';

                // Case 1: Full order (has qty and preference if needed)
                if (qty && (!needsSpicy || preference)) {
                    // Add to cart with preference
                    const itemWithPreference = preference ? { ...item, preference } : item;
                    for (let i = 0; i < qty; i++) {
                        cart.push(itemWithPreference);
                    }

                    setBotState(prev => ({ ...prev, cart: [...cart], currentItem: null }));

                    const itemName = preference ? `${item.name.en} (${preference})` : item.name.en;
                ...
                // Case 2: Has qty but needs preference
                if (qty && needsSpicy && !preference) {
                    setBotState(prev => ({
                        ...prev,
                        step: 'ITEM_PREFERENCE',
                        currentItem: { ...item, qty }
                    }));

                    addMessage(`How would you like your ${item.name.en}?`, 'bot', 'button', [
                        { id: 'pref_spicy', label: 'Spicy' },
                        { id: 'pref_nonspicy', label: 'Non-Spicy' }
                    ]);
                    return;
                }

                // Case 3: Has preference but needs qty
                if (preference && !qty) {
                    setBotState(prev => ({
                        ...prev,
                        step: 'ITEM_QTY',
                        currentItem: { ...item, preference }
                    }));

                    addMessage(`How many ${item.name.en} (${preference}) would you like?`, 'bot', 'button', [
                        { id: 'qty_1', label: '1' },
                        { id: 'qty_2', label: '2' },
                        { id: 'qty_3', label: '3' },
                ...
                // Case 4: Needs preference first (burgers/wraps)
                ...
                // Case 5: Just needs qty (no preference needed)
                setBotState(prev => ({
                    ...prev,
                    step: 'ITEM_QTY',
                    currentItem: item
                ...
                addMessage(`How many ${item.name.en} would you like?`, 'bot', 'button', [
                    { id: 'qty_1', label: '1' },
                    { id: 'qty_2', label: '2' },
                    { id: 'qty_3', label: '3' },
                    { id: 'qty_more', label: 'More' }
        ...
        // Handle generic menu requests (menu, show menu, what do you have, etc.)
        const menuRegex = /\b(menu|show me the menu|what do you have|i want to order|need your menu|menu\?+|menu please|what's on the menu|see the menu|see menu|can i see the menu|can i get the menu|display menu|give me the menu|menu options|menu items|menu list|menu card|menu now|menu info|menu information|menu details|menu\s*)\b/i;
        if (

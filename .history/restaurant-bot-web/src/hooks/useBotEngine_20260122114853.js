...
export function useBotEngine() {
    ...
    const parseNaturalLanguageOrder = (text, lang) => {
        ...
        const stopWords = ['please', 'add', 'give', 'me', 'want', 'need', 'like', 'would', 'can', 'i', 'get', 'have', 'the', 'a', 'an', 'some', 'any', 'order', 'to', 'من', 'فضلك', 'أريد', 'أضف'];
        
        // Item modifiers that should be kept with the item name
        const itemModifiers = ['beef', 'chicken', 'spicy', 'non', 'hot', 'cold', 'large', 'small', 'medium'];
        ...
        // Pattern 1: Quantity + item/category ("2 burgers", "٢ برجر", "2 beef burgers")
        // Enhanced to support both Latin and Arabic text, and multi-word items
        const intentRegex = new RegExp(`(${numberPattern})\\s+(?:of\\s+|for\\s+|add\\s+|give\\s+me\\s+|من\\s+|أضف\\s+)?([a-zA-Z\\u0600-\\u06FF]+(?:[\\s-][a-zA-Z\\u0600-\\u06FF]+){0,3})(?=\\s*(?:\\d|and|&|,|\\+|و|$|\\?))`, 'gi');
        ...
        if (step !== 'ITEM_QTY' && step !== 'ITEM_PREF' && step !== 'ITEM_CONFIRM' && step !== 'LANGUAGE') {
            ...
            if (isIrrelevantQuestion(text)) {
                const politeMsg = language === 'en'
                    ? "Sorry, I cannot assist with that.\n\n📋 Let me show you our menu instead:"
                    : "عذراً، لا أستطيع المساعدة في ذلك.\n\n📋 دعني أريك قائمتنا بدلاً من ذلك:";

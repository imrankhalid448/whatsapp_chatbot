    ...
    const processInput = (text) => {
            ...
            if (text.startsWith('add_')) {
                ...
                if (selectedItem) {
                    ...
                    if (pendingResolution) {
                        ...
                        const confirmMsg = language === 'en'
                            ? `Adding ${selectedQty}x ${selectedItem.name[language]}...`
                            : `إضافة ${selectedQty}x ${selectedItem.name[language]}...`;
                        addMessage(confirmMsg, 'bot');
        ...
        if (step === 'ITEM_QTY') {
            if (text === 'type_qty' || text === t.typeQuantity) {
                ...
                const inputMsg = language === 'en'
                    ? "📝 Just type the number (e.g., 5 or ten):"
                    : "📝 اكتب الرقم فقط (مثال: 5 أو عشرة):";
                addMessage(inputMsg, 'bot');
                return;
            }
        ...
        if (step === 'QTY_INPUT') {
            ...
            if (!isNaN(qty) && qty > 0) {
                ...
                const errorMsg = language === 'en'
                    ...
                    : "❌ الرجاء إدخال رقم صحيح (مثال: 1، 2، 5)";
                    ...
                    if (remainingCategories.length > 0) {
                        ...
                        setTimeout(() => {
                            const politeConfirmMsg = language === 'en'
                                ? `✅ Excellent, sir! Your ${currentCategoryData?.title[language]} order is now complete.\n\n📋 You requested ${categoryQueue.map(c => `${c.qty}x ${c.data.title[language]}`).join(' and ')}.\n\nShall we proceed with ${nextCategory.data.title[language]}?`
                                : `✅ ممتاز يا سيدي! طلب ${currentCategoryData?.title[language]} اكتمل الآن.\n\n📋 طلبت ${categoryQueue.map(c => `${c.qty}x ${c.data.title[language]}`).join(' و ')}.\n\nهل نتابع مع ${nextCategory.data.title[language]}؟`;
                            
                            addMessage(politeConfirmMsg, 'bot', 'button', [
                                { id: 'proceed_next_category', label: language === 'en' ? '✅ Yes, Proceed' : '✅ نعم، تابع' },
                                { id: 'order_more_items', label: language === 'en' ? '🛒 Order More' : '🛒 طلب المزيد' },
        ...
        const handleUnknownInput = (currentStep, lang) => {
            ...
            if (currentStep === 'ITEM_QTY' || currentStep === 'QTY_INPUT') {
                const qtyMsg = lang === 'en'
                    ? "🔢 Please enter a number or select from the buttons (1, 2, 3, 4...)"
                    : "🔢 الرجاء إدخال رقم أو تحديد من الأزرار (1، 2، 3، 4...)";

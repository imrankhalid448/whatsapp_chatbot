    ...
    const findBestMatchItem = (text, lang) => {
        ...
        allItems.forEach(item => {
            ...
            // 3. Fuzzy match with dynamic threshold based on word length
                ...
                const prefixMatches = normalizedSearch.substring(0, prefixLen) === normalizedNameEn.substring(0, prefixLen);
                const adjustedDist = prefixMatches ? dist - 1 : dist; // Give bonus if prefix matches
                
                if (dist <= threshold && adjustedDist < minDist) {

...
export function useBotEngine() {
    ...
    const findBestMatchItem = (text, lang) => {
        ...
        allItems.forEach(item => {
            ...
            if (nameEn === rawSearch || nameEn === normalizedSearch) {
                bestMatch = item;
                bestScore = -1000; // Guaranteed best
                return;
            }
            ...
            if (nameEn.includes(rawSearch) || nameAr.includes(text) ||
                ...
                const score = -500; // Very high priority
                if (score < bestScore) {
                    bestMatch = item;
                    bestScore = score;
                }
                return; // Don't check fuzzy for this item
            }
            ...
            const maxPrefixLen = Math.min(4, Math.floor(searchLen * 0.6));
            ...
            score -= prefixMatchLen * 50; // Strong bonus for each matching prefix char
            
            // Length similarity bonus: penalize if lengths are very different
            const lenDiff = Math.abs(normalizedSearch.length - normalizedNameEn.length);
            score += lenDiff * 10;
            
            // First character match bonus (important for "burer" vs "water")
    ...
    const findBestMatchCategory = (text, lang) => {
        ...
        menu.categories.forEach(c => {
            ...
            // Prefix matching bonus: if first 2-3 chars match, prioritize this match
            const prefixLen = Math.min(3, Math.floor(searchLen / 2));
            const prefixMatches = normalizedSearch.substring(0, prefixLen) === normalizedCat.substring(0, prefixLen);
            const adjustedD = prefixMatches ? d - 1 : d; // Give bonus if prefix matches
            
            if (d <= threshold && adjustedD < min) {
                min = adjustedD;
                match = c;
            }
        });
    ...
    const parseNaturalLanguageOrder = (text, lang) => {
        ...
        while ((match = itemQtyRegex.exec(cleanText)) !== null) {
            ...
            // Check if already detected using normalized comparison
            const alreadyDetected = detectedIntents.some(intent => {
                if (intent.type === 'CATEGORY') {
                    const normalizedCat = normalizeText(intent.data.title.en);
                    return normalizedCat.includes(normalizedText) || 
                           normalizedText.includes(normalizedCat);
                }
                if (intent.type === 'ITEM') {
                    const normalizedItem = normalizeText(intent.data.name.en);

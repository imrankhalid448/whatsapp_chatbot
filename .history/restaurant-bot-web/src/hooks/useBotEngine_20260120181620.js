...
export function useBotEngine() {
    ...
    const findBestMatchItem = (text, lang) => {
        ...
        const threshold = 4; // Allow more typos
    ...
    const findBestMatchCategory = (text, lang) => {
        ...
        // 2. Partial match (e.g., "wrap" in "wraps")
        ...
        // 3. Fuzzy match (spell mistakes)
        let min = Infinity;
        const threshold = 4; // Allow more typos for categories
        menu.categories.forEach(c => {
            const catEn = c.title.en.toLowerCase();
            const d = levenshtein(search, catEn);
            
            // For short words (<=5 chars), allow distance of 2
            // For longer words, allow distance of 4
            const maxDist = search.length <= 5 ? 2 : threshold;
    ...
    const parseNaturalLanguageOrder = (text, lang) => {
        ...
        while ((match = intentRegex.exec(cleanText)) !== null) {
            ...
            // Check Category FIRST (to avoid false item matches for category names)
            // Especially for short words like "wrap", "drink", "side", etc.
            ...
            // If not a category, try Specific Item
    ...
    const processInput = (text) => {
        ...
        if (step !== 'ITEM_QTY' && step !== 'ITEM_PREF' && step !== 'ITEM_CONFIRM' && step !== 'LANGUAGE' && step !== 'PAYMENT_METHOD' && step !== 'ORDER_SUMMARY') {
            ...
            const finishPhrases = [
                'finish', 'complete', 'done', 'no more', 'not any more', 
                'that\\'s all', 'thats all', 'i\\'m done', 'im done',
                'complete order', 'finish order', 'checkout', 'check out',
                'proceed to payment', 'pay now', 'ready to pay',

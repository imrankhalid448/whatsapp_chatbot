...
// --- HELPER: Item aliases and common misspellings for better matching ---
const getItemAliases = () => {
    return {
        // Burgers
        'chicken-burger': ['chicken', 'chiken', 'chikin', 'chikn', 'chickin', 'chcken', 'burger'],
        'beef-burger': ['beef', 'bef', 'beaf', 'meet', 'meat', 'burger'],
        'double-burger': ['double', 'dubble', 'doble', 'dubl', 'burger'],
        'special-burger': ['special', 'spicial', 'spesial', 'specal', 'burger'],
        
        // Wraps
        'chicken-wrap': ['chicken', 'chiken', 'chikin', 'wrap', 'roll'],
        'beef-wrap': ['beef', 'bef', 'beaf', 'wrap', 'roll'],
        'falafel-wrap': ['falafel', 'falafil', 'flafel', 'wrap', 'roll'],
        
        // Sandwiches
        'egg-sandwich': ['egg', 'eg', 'sandwich', 'sandwitch', 'sandwch'],
        'cheese-sandwich': ['cheese', 'chese', 'cheez', 'sandwich'],
        'tuna-sandwich': ['tuna', 'toona', 'tona', 'sandwich'],
        'kibdah-sandwich': ['kibdah', 'kebdah', 'kabdah', 'liver', 'sandwich'],
        
        // Sides
        'french-fries': ['fries', 'frys', 'fris', 'french', 'potato', 'potatoes'],
        'onion-rings': ['onion', 'onyon', 'rings', 'ring'],
        'nuggets': ['nugget', 'nuget', 'nuggit', 'nugits', 'chicken'],
        'mozzarella-sticks': ['mozzarella', 'mozarella', 'mozarela', 'cheese', 'sticks'],
        
        // Meals
        'family-meal': ['family', 'famly', 'meal', 'deal'],
        'combo-meal': ['combo', 'combu', 'kombo', 'meal', 'deal'],
        'value-meal': ['value', 'valyu', 'meal', 'deal'],
        
        // Juices
        'orange-juice': ['orange', 'oranje', 'orang', 'juice'],
        'mango-juice': ['mango', 'manga', 'mangu', 'juice'],
        'strawberry-juice': ['strawberry', 'strawbery', 'strawbry', 'juice'],
        'cocktail-juice': ['cocktail', 'coctail', 'koktel', 'juice', 'mix'],
        
        // Drinks
        'pepsi': ['pepsi', 'pepsy', 'pesi', 'cola', 'coke'],
        'water': ['water', 'watter', 'wter', 'h2o'],
        'coffee': ['coffee', 'coffe', 'cofee', 'koffee', 'cafe'],
        'tea': ['tea', 'tee', 'te', 'chai'],
        'laban': ['laban', 'lban', 'labn', 'yogurt', 'yoghurt']
    };
};
...
export function useBotEngine() {
    ...
    const findBestMatchItem = (text, lang) => {
        ...
        // Get item aliases for better matching
        const itemAliases = getItemAliases();
        ...
        allItems.forEach(item => {
            ...
            // Get aliases for this item if they exist
            ...
            for (const alias of aliases) {
                if (alias === rawSearch || alias === normalizedSearch ||
            ...
            // 3. Fuzzy match with comprehensive scoring - RELAXED THRESHOLDS
            let threshold;
            if (searchLen <= 4) threshold = 3;      // Increased from 2 - allows "bef" → "beef"
            else if (searchLen <= 7) threshold = 4; // Increased from 3 - allows "burer" → "burger"
            else threshold = 6;                     // Increased from 5 - very lenient for long words

            const distEn = levenshtein(normalizedSearch, normalizedNameEn);
            const distRaw = levenshtein(rawSearch, nameEn);
            let dist = Math.min(distEn, distRaw);
            
            // Also check against aliases
            for (const alias of aliases) {
                const normalizedAlias = normalizeText(alias);
                const aliasDist = Math.min(
                    levenshtein(normalizedSearch, normalizedAlias),
                    levenshtein(rawSearch, alias)
                );
            ...
            const maxPrefixLen = Math.min(5, Math.floor(searchLen * 0.7));
            ...
            score -= prefixMatchLen * 70; // Increased bonus from 60
            
            // Length similarity: reduced penalty for different lengths
            const lenDiff = Math.abs(searchLen - itemLen);
            score += lenDiff * 8; // Reduced from 15
        ...
        // More lenient confidence threshold - increased from 300 to 500
        return (bestMatch && bestScore < 500) ? bestMatch : null;
    };
    ...
    const parseNaturalLanguageOrder = (text, lang) => {
        ...
        const stopWords = ['please', 'add', 'give', 'me', 'want', 'need', 'like', 'would', 'can', 'i', 'get', 'have', 'the', 'a', 'an', 'some', 'any', 'من', 'فضلك', 'أريد', 'أضف'];
        ...
        // Pattern 1: Quantity + item/category ("2 burgers", "٢ برجر")
        // Enhanced to support both Latin and Arabic text
        const intentRegex = new RegExp(`(${numberPattern})\\s+(?:of\\s+|for\\s+|add\\s+|give\\s+me\\s+|من\\s+|أضف\\s+)?([a-zA-Z\\u0600-\\u06FF]+(?:[\\s-][a-zA-Z\\u0600-\\u06FF]+)*)(?=\\s*(?:\\d|and|&|,|\\+|و|$|\\?))`, 'gi');
        ...
        while ((match = intentRegex.exec(cleanText)) !== null) {
            ...
            let matched = false;
            const likelyCategory = isLikelyCategory(rawText);

            // If text looks like a category, try category first
            if (likelyCategory) {
